...
<div class="modal-dialog no-modal-header" role="document"><div class="modal-content"><div class="modal-body">
    ...
    <div id="wrapper" style="font-size: 0.7em !important;>
        <div id="receiptData">
            <div class="no-print">
                <?php
                if ($message) {
            ...
            <div id="receipt-data">
                ...
                //     }
                // }
                // echo lang('sales_person') . ': ' . $created_by->first_name . '';
                echo '<p>';
                // echo lang('customer') . ': ' . ($customer->company && $customer->company != '-' ? $customer->name : $customer->name) . '<br>';
                ...
                if ($return_payments) {
                    ...
                    foreach ($return_payments as $payment) {
                        ...
                        if (($payment->paid_by == 'cash' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
                            ...
                            echo '<td colspan="2">' . lang($payment->paid_by) . '</td>';
                            echo '<td colspan="2">' . $this->sma->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
                            // echo $payment->note ? '</tr><td colspan="4">' . lang('payment_note') . ': ' . $payment->note . '</td>' : '';
                        }
                        echo '</tr>';
                    }
